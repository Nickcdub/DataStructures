   ...
   private MyArrayList<T> list;
    ...
    private boolean binarySearch(T item, int start, int finish) {
       ...
       return false;
        ...
        /* comparisons++;
        if (start <= finish && item.compareTo( list.get(start)) != 0 || start>finish) return false;
        int mid = (finish - start) / 2 + start;

        comparisons++;
        if (item.compareTo( list.get(mid)) == 0) return true;

        comparisons++;
        if (item.compareTo( list.get(mid)) < 0) return binarySearch(item, start, mid - 1);
